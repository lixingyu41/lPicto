...
分页规则：`page` 从 1 开始，`pageSize` 使用应用内置默认值和上限。分页响应：
...
LIB 路径是相对 `/Media` 的路径；例如 `/Media/电影/2024` 对应 `电影/2024`，空字符串表示全部媒体根。
...
- `GET /api/albums/:id/anchors?pageSize=100&sort=timeline_desc&q=`
- `GET /api/albums/source-folders?parentRelPath=`
- `GET /api/album-groups`
- `POST /api/album-groups`
